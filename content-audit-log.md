...
- **修复Games分类4页共享桥接骨架**（今日dice-roller独立审核发现的通用问题，见`内容发布日志.md`9/22条目"遗留①"）：coin-flip-simulator/click-speed-test/dice-roller/reaction-time-test四页coreSummary结尾此前全部共用"A different kind of X...is what[链接]...instead/measures"这一修辞骨架，`check_bridge_sentences.py`只查桥接句事实真实性和单页内重复，不查跨页共享骨架，属机械检查未覆盖的维度。四句全部改写为结构互不相同的开头（以链接工具作主语/以目的状语开头等），事实断言不变；click-speed-test→coin-flip-simulator和dice-roller→coin-flip-simulator两条被`check_bridge_sentences.py`标记为候选，逐条核对coin-flip-simulator原文（binomial公式精确概率、卷积等价关系）属实后用`--verify`回填CONFIRMED。全站52/52页`check_prose_patterns.py`通过、`npm test`74/74文件1385/1385用例、`npm run build`125页0 errors。commit`7dd060d`推送，curl绕缓存核实两页线上生效，`node tools/submit-indexnow.mjs`提交4个URL Bing/Yandex均200（commit`6484a2b`）。
- **判通用/扩散判定**：判为**通用**（同一修辞骨架大概率也在站内其他分类复用，非Games分类独有），本次仅在有独立发现的Games分类4页范围内修复，未做全站51页扫描（现有`check_bridge_sentences.py`/`check_bridge_template_reuse.py`均不检测"跨页共享骨架"这个维度，需要新的检测逻辑才能全站扫描，非本次能顺手完成的范围），未新增机械检查项防复发。如需扩大到全站或接入检查防复发，需专门排期，本条如实记录未做而非假装已覆盖。
